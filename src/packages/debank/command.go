...
func (c *Command) Execute(noCondition bool) (string, error) {
	...
	if debt.IsPositive() {
		...
		if noCondition {
			...
			return fmt.Sprintf("%v\n<strong>V:%v | Pre: %v | L:%v | H:%v</strong>", c.Name, u.ShortenDecimal(debt, int32(precision), 2), u.ShortenDecimal(_prev, int32(precision), 2), u.ShortenDecimal(c.low, int32(precision), 2), u.ShortenDecimal(c.high, int32(precision), 2)), nil
		}
	}
	return "", nil
}
